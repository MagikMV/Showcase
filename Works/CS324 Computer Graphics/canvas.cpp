...
Canvas::Canvas(size_t w, size_t h, color bg) :
    width(w), height(h),
    pixels(width * height, {bg}),
    background(bg) { }

void Canvas::SetColor(size_t x, size_t y, color c)
{
    if(x >= 0 && x < width && y >= 0 && y < height)
	pixels[y * width + x] = {c};
}

pixel Canvas::GetPixel(size_t x, size_t y) const
{
    return pixels[y * width + x];
}

void Canvas::SetPixel(size_t x, size_t y, pixel p)
{
    if(x >= 0 && x < width && y >= 0 && y < height)
	pixels[y * width + x] = p;
}
