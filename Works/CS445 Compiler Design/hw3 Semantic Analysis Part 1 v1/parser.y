...
varDeclInit: varDeclId								{ $$ = $1; }
		| varDeclId ':' simpleExp					{ $1->child[0] = $3; $$ = $1; $$->initialized = true; }
		;
...
exp		: mutable '=' exp							{ $$ = newExpNode(AssignK, $2, $1, $3); $$->equalSign = true; }
		| mutable ADDASS exp						{ $$ = newExpNode(AssignK, $2, $1, $3); $$->equalSign = true; }
		| mutable SUBASS exp						{ $$ = newExpNode(AssignK, $2, $1, $3); $$->equalSign = true; }
		| mutable MULASS exp						{ $$ = newExpNode(AssignK, $2, $1, $3); $$->equalSign = true; }
		| mutable DIVASS exp						{ $$ = newExpNode(AssignK, $2, $1, $3); $$->equalSign = true; }
		| mutable INC								{ $$ = newExpNode(AssignK, $2, $1); }
		| mutable DEC								{ $$ = newExpNode(AssignK, $2, $1); }
		| simpleExp									{ $$ = $1; }
		;

simpleExp: simpleExp OR andExp						{ $$ = newExpNode(OpK, $2, $1, $3); $$->boolbool = true; }
		| andExp									{ $$ = $1; }
		;

andExp	: andExp AND unaryRelExp					{ $$ = newExpNode(OpK, $2, $1, $3); $$->boolbool = true; }
		| unaryRelExp								{ $$ = $1; }
		;

unaryRelExp: NOT unaryRelExp						{ $$ = newExpNode(OpK, $1, $2); $$->unary = true; }
		| relExp									{ $$ = $1; }
		;
    ...
    while(1)
    {
        ...
        if(optind < argc)
		{
			...
			else
			{
				...
				// ERROR(ARGLIST): source file\"%s\" could not be opened.
				// numErrors++ and printf number of errors and warnings
				exit(1);
			}
		}
		else
